...
class Haxcan {
	...
	public static function get_ajax_response() {		
		...
		switch ( $_POST['_action_request'] ) {
			...
			$haxfile = sanitize_text_field($_POST['haxfile']);
			$fajl = file($haxfile);
			wp_send_json($fajl);
			break;
			case 'save_scan':
			...
			update_option('haxcan_tfs', sanitize_text_field($_POST['theme_files_scanned']));
			break;
			...
			update_option('haxcan_last_scan', current_time('mysql'));
			update_option('haxcan_pfs', sanitize_text_field($_POST['plugin_files_scanned']));
			break;
			case 'add_to_quarantine':
			global $wpdb;
			$file = sanitize_text_field($_POST['theme_file']);
			$reason = sanitize_text_field($_POST['reason']);
			$lajn = sanitize_text_field($_POST['line']);
			$wpdb->insert( 
			...
			case 'check_theme_file':
				if ( ! empty( $_POST['_theme_file'] ) ) {
					$theme_file = sanitize_text_field(wp_unslash( $_POST['_theme_file'] ));
			...
			<div class="haxcan-wrap">
				<div class="haxcan-header">
					...
					<div class="haxcan-logo haxcan-panel-change" id="nav-01">
						<img src="<?php echo esc_url( plugins_url( 'admin/images/haxcan-logo.svg', dirname(__FILE__ )) );?>" alt="haxcan" />
					...
					<div class="haxcan-infoboxes">
						<div class="haxcan-infobox" id="haxcan-status">
							...
							<h2 class="haxcan-active" id="haxcan-general-status">
								<?php if(!$lastscanid&&!$lastqid){
									?>
									<div style="color:#f78c00">Scan Needed</div>
									<?php } 
									if($lastqid){
										?>
										<div style="color:#fc1303">Insecure</div>
										<?php
									}
						...
						<div class="haxcan-infobox">
							<h5>LAST SCAN</h5>
							<h2 id="haxcan-last-scans"><?php if(!$lastscanid){ echo '-';} else { echo esc_attr(self::time_elapsed_string(get_option('haxcan_last_scan')));}?></h2>
						</div>
						...
						<div class="haxcan-infobox">
							<h5>HTTPS</h5>
							<h2 class="haxcan-active"><?php if(is_ssl()){?>
								Enabled
								<?php } else { ?>
							<div style="color:#f78c00">Disabled</div><?php }?></h2>
						</div>
						
					</div>
					<div class="haxcan-full-scan">
						<a href="https://wordpress.org/support/plugin/haxcan/reviews/#new-post" target="_blank" class="haxcan-button">Rate us on WP!</a>
					</div>
					<div class="haxcan-icon-settings">
						<div class="haxcan-quarantin haxcan-tooltip haxcan-panel-change" id="nav-03">
							<span id="haxcan-notifs" <?php if($lastqid){?> style="display:block;"<?php } ?>><?php if($lastqid){ echo esc_attr($lastqid);} else { echo '0';}?></span>
							<img src="<?php echo esc_url( plugins_url( 'admin/images/quarantin.svg', dirname(__FILE__ )) );?>" alt="quarantin" />
							<span class="haxcan-tt">Quarantine</span>
						</div>
						<div class="haxcan-settings haxcan-tooltip haxcan-panel-change" id="nav-02">
							<img src="<?php echo esc_url( plugins_url( 'admin/images/settings.svg', dirname(__FILE__ )) );?>" alt="quarantin" />
				...
				<button class="haxcan-dropbtn"><span id="haxcan-dashboard">Dashboard</span> <img src="<?php echo esc_url( plugins_url( 'admin/images/chevron-down.svg', dirname(__FILE__ )) );?>" alt="" /></button>
					...
					<div class="haxcan-content-container" id="haxcan-content-01">
						...
						<div class="haxcan-widget" id="haxcan-theme-scan">
							<h2 class="haxcan-widget-title"><img src="<?php echo esc_url( plugins_url( 'admin/images/file.svg', dirname(__FILE__ )) );?>" alt="quarantin" /> Themes Scanned</h2>
							<div class="haxcan-widget-content">
								<?php
								if(!$lastscanid){ ?>
									<div class="haxcan-no-scans"><img class="tpDanger" src="<?php echo esc_url( plugins_url( 'admin/images/danger.svg', dirname(__FILE__ )) );?>" alt="" /><p>No scans found. Press button under to start all themes scanning.</p></div>
								...
								<div class="haxcan-loading-data" style="display:block">
									<div class="haxcan-wrap-top">
									<div id="haxcan-total-plugins" class="haxcan-tp">
								<?php echo esc_attr($arr['num_of_themes']).'/'.esc_attr($arr['num_of_themes']);?>
									</div>
									...
									<div id="haxcan-donut">
										...
										<div class="haxcan-legend">
											<div id="haxcan-themes-fs"><div class="haxcan-green-dot"></div> <span id="haxcan-files-scanned"><?php echo esc_attr($totalfilescount);?></span> Scanned files</div>
									...
									<div class="haxcan-current-scan">
										...
										<div id="haxcan-current-tf" <?php if($arr['num_of_themes']>1){ ?>class="haxcan-mt"<?php }?>>
											<?php
											$curr = 0;
											foreach($allthemes as $currtheme){
												$out = strlen($currtheme->get('Name')) > 40 ? substr($currtheme->get('Name'),0,40)."..." : $currtheme->get('Name');?>
											<h4><?php echo esc_attr($currtheme->get('Name'));?> <img src="<?php echo esc_url( plugins_url( 'admin/images/check.svg', dirname(__FILE__ )) );?>" alt="" /></h4>
												<?php if($curr==1){ ?>
													<img src="<?php echo esc_url( plugins_url( 'admin/images/expand.png', dirname(__FILE__ )) );?>" alt="expand" onClick="haxcanThemeExpand()" class="haxcan-expander"/>
													<?php }
												$curr++;
											}
											?>
								...
								<div class="haxcan-footer">
									...
									<?php if(!$lastscanid){?>Start Scan<?php } else {?>Scan Again<?php }?>
								</a>
							</div>
							</div>
						</div>
						...
						<div class="haxcan-widget" id="haxcan-plugin-scan">
							<h2 class="haxcan-widget-title"><img src="<?php echo esc_url( plugins_url( 'admin/images/file.svg', dirname(__FILE__ )) );;?>" alt="quarantin" /> Plugins Scanned</h2>
							<div class="haxcan-widget-content">
								<?php
								if(!$lastscanid){ ?>
									<div class="haxcan-no-scans"><img class="tpDanger" src="<?php echo esc_url( plugins_url( 'admin/images/danger.svg', dirname(__FILE__ )) );?>" alt="" /><p>No scans found. Press button under to start all plugins scanning.</p></div>
								...
								<?php echo $plugincount.'/'.$plugincount;?>
									...
									<div id="haxcan-donut-2">
										...
										<div class="haxcan-legend">
											<div id="haxcan-themes-fs"><div class="haxcan-green-dot"></div> <span id="haxcan-files-scanned-2"><?php echo esc_attr($pluginfilescount);?></span> Scanned files</div>
											<div id="haxcan-themes-fr"><div class="haxcan-gray-dot"></div> <span id="haxcan-files-remaining-2">0</span> Files remaining</div>
										</div>	
										...
										<div id="haxcan-current-tf-2" <?php if($arr['num_of_themes']>1){ echo 'class="haxcan-mt"';}?>>
											...
											if($plugincount){
											    foreach ( $plugin_array as $pluginfile => $plugindata) {
													...
													?>
													<h4><?php echo esc_attr($out);?> <img src="<?php echo esc_url( plugins_url( 'admin/images/check.svg', dirname(__FILE__ )) );?>" alt="" /></h4>
													<?php
													if($curr==1){ ?>
												   <img src="<?php echo esc_url( plugins_url( 'admin/images/expand.png', dirname(__FILE__ )) );?>" alt="expand" onClick="haxcanPluginExpand()" class="haxcan-expander" />
													<?php
													}
													$curr++;
											    }
					...
					<div class="haxcan-content-container" id="haxcan-content-03">
						...
						<?php } 
							...
							if(count($result)){
								foreach($result as $quar){?>
									<div class="haxcan-quarantine-item">
									<a href="javascript:haxcanLoadfile('<?php echo esc_attr($quar->filepath);?>')"><?php echo esc_attr($quar->filepath);?></a>
									<span class="haxcan-time">Scan time: <?php echo esc_attr($quar->time_added);?></span><span class="haxcan-time">Line: <?php echo esc_attr($quar->atline);?></span>
									<span class="haxcan-time">Code found: <div style="color:red;display:inline"><?php echo esc_attr($quar->reason);?></div></span></div>
									<?php }
						}
						?>
					</div>
